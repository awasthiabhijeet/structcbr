...
@DatasetReader.register("cbr_concat_roberta")
class CBRConcatRoberta(DatasetReader):
    def __init__(
        self,
        lazy: bool = True,
    ...
    def text_to_instance(
        ...
        schema_tokens = [
            ...
            for x in [self._tokenizer.tokenize(x)[1:-1] for x in schema_tokens_pre] #NOTE:  changed because bigbird doesnt add any token at the end(this was fixed by 2 sep token, actually it adds but Token type id doesnt allow take last 2 tokens to be returned), use above line for grappa
